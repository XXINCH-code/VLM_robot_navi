...
class Config(object):
    ...
    env.csl_workspace_type = 'corridor'  # hallway, lounge, corner, corridor
    # sim or sim2real
    env.mode = 'sim2real'
    ...
    if env.scenario == 'csl_workspace':
        ...
        if env.csl_workspace_type == 'hallway':
            ...
            human_flow.static_regions = np.array([[-175, -75, 1225, 1400], [30, 100, 750, 950],
                                                  ]) / 100.
            
            # will be triggered ONLY IF sim.static_obs = True and sim.random_obs = False
            # key: region number, value: [x_low, x_high, y_low, y_high] of the rectangular shaped region
            human_flow.regions = {0: np.array([400, 500, 1200, 1350]) / 100.,
    ...
    training.output_dir = 'data/ours_RH_HH_corridorEnv'  # the saving directory for train.py
